...
if (sitePath === '/') {
  ...
  const additionalContent = pageKey === 'maganszemelyeknek'
    ? `<section class="process"><div><p class="eyebrow">${language === 'hu' ? 'Munkafolyamat' : 'Our process'}</p><h2>${language === 'hu' ? 'Tervezéstől a telepítésig' : 'From planning to installation'}</h2></div><div class="process-steps"><article><span>01</span><h3>${language === 'hu' ? 'Konzultáció és tervezés' : 'Consultation and planning'}</h3><p>${language === 'hu' ? 'Megbeszéljük az igényeket, felmérjük a helyszínt, és közösen alakítjuk ki a legjobb megoldást.' : 'We discuss requirements, survey the site, and develop the most suitable solution together.'}</p></article><article><span>02</span><h3>${language === 'hu' ? 'Gyártás és telepítés' : 'Production and installation'}</h3><p>${language === 'hu' ? 'A szerkezeteket a projekt igényei szerint készítjük, majd pontosan és egyeztetett időben telepítjük.' : 'We produce structures for the project and install them precisely at an agreed time.'}</p></article></div><p class="service-area">${language === 'hu' ? 'Nagyobb projekteket országosan vállalunk. Kiemelt szolgáltatási területeink: Budapest és Pest környéke, Komárom-Esztergom, valamint Fejér megye.' : 'We undertake larger projects nationwide, with priority service areas in Budapest and Pest, Komarom-Esztergom, and Fejer county.'}</p></section>`
    : ''
  document.querySelector('#app').innerHTML = `<header class="site-header"><a class="brand" href="/" aria-label="VasAtis"><img src="/assets/vasatis-logo.svg" alt="VasAtis"></a><nav>${text.navigation.map((item) => `<a href="${item.href}">${item.label}</a>`).join('')}</nav></header><main><section class="page-hero"><p class="eyebrow">VASATIS KFT.</p><h1>${data[0]}</h1><p>${data[1]}</p></section>${content}${additionalContent}</main><footer><p>${text.footer.summary}</p><p>${text.footer.address}</p><p><a href="/adatvedelmi-tajekoztato/">${text.footer.privacy}</a> | <a href="/kapcsolat/">${text.footer.contact}</a></p></footer>`
