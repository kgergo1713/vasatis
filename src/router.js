...
if (sitePath === '/') {
  ...
  document.querySelector('#app').innerHTML = `<header class="site-header"><a class="brand" href="/" aria-label="VasAtis"><img src="/assets/vasatis-logo.svg" alt="VasAtis"></a><nav>${text.navigation.map((item) => `<a href="${item.href}">${item.label}</a>`).join('')}</nav></header><main><section class="page-hero"><p class="eyebrow">VASATIS KFT.</p><h1>${data[0]}</h1><p>${data[1]}</p></section>${content}</main><footer><p>${text.footer.summary}</p><p>${text.footer.address}</p><p><a href="/adatvedelmi-tajekoztato/">${text.footer.privacy}</a> | <a href="/kapcsolat/">${text.footer.contact}</a></p></footer>`
